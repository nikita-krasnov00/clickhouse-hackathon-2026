...
//   - planning несёт список запланированных карточек (title + source) — LLM
//     теперь строит дашборд из нескольких карточек и может брать готовые
//     дриллы каталога A4 как инструменты;
...
//   - card_ready несёт ОДНУ готовую карточку (+ её SQL) — UI рендерит её
//     сразу, не дожидаясь конца рана (прогрессивная загрузка);
...
export const runStepSchema = z.discriminatedUnion("step", [
  ...
  z.strictObject({
    step: z.literal("planning"),
    /** Запланированные карточки: заголовок + источник ('sql' | 'drill:<id>'). */
    cards: z
      .array(z.strictObject({ title: z.string(), source: z.string() }))
      .optional(),
    message: z.string().optional(),
  }),
  z.strictObject({
