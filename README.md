...
TRIGGER_SECRET_KEY_PROD=tr_prod_… npm run vercel:env   # upload env from .env to production
npx vercel deploy --prod
```

`vercel:env` (scripts/vercel-env-push.sh) uploads the ClickHouse + LLM +
Auth (Google OAuth) variables from `.env`; `TRIGGER_SECRET_KEY_PROD` is the **prod**
Trigger.dev key (`tr_prod_…`, dashboard → API Keys), passed separately so that the dev key from `.env`
doesn't end up in prod. Don't forget the prod domain in the OAuth client's Authorized redirect URIs
(the "Google OAuth" section above).
